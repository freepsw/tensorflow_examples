...
#    - output, state : 다음 단계에서 활용할 output(size=4), state 생성
...
# targets: 실제값, list of 1D batch-sized int32 Tensors of the same length as logits.
...
# 3-2) output객체를 1차원 배열로 변경해 준다(concat) --> 차원만 변경함. shape(1,16)
#      reshape를 실행하기 위한 용도
